...
const Sidebar = () => {
    ...
    const handleUserSelect = (user) => {
        setSelectedUser(user);
        if (isMobile) {
            setIsSidebarOpen(false); // Close the sidebar on mobile when a user is selected
        }
    };
    ...
    return (
        <aside className={`h-full w-20 lg:w-72 border-r border-base-300 flex flex-col transition-all duration-200 ${!isSidebarOpen ? "hidden" : ""}`}>
            <div className="border-b border-base-300 w-full p-5">
                <div className="flex items-center gap-2">
                    <Users className="size-6" />
